...
- For local Docker Compose runs, `docker-compose.yml` explicitly points `LOTUS_CORE_BASE_URL` to `http://core-query.dev.lotus` and `LOTUS_PERFORMANCE_BASE_URL` to `http://performance.dev.lotus`, with `extra_hosts` mapping those canonical names back to the local host gateway.
