...
6. `lotus-idea` RFC-0002 Slice 16/17 may consume the source-safe producer proof generated by
   `make idea-opportunity-runtime-evidence`; the artifact clears only Risk source-proof blockers
   and does not transfer risk methodology, data-mesh, Gateway/Workbench, client-publication,
   deployment, production, or supported-feature authority.
