...
6. `lotus-core` must be consumed as a governed source-data, analytics-input, snapshot/simulation, and support-metadata authority, while `lotus-performance` remains the authority for performance return and benchmark exposure context inputs — and, since the lotus-risk#291 consumer slice, for per-group return evidence consumed from its contribution surface (`/performance/contribution`, `ContributionRow.group_return`): stateful `TOTAL_RISK` attribution requests that evidence per resolved period for `SECTOR`/`ASSET_CLASS`/`POSITION` groupings, decomposes it only when validated evidence covers every portfolio return date (a date absent from a group's evidence is unknown, never zero; nothing is zero-filled or dropped), reports the per-set basis in `risk_basis`, refuses malformed evidence as `UPSTREAM_INVALID_RESPONSE`, and keeps `ACTIVE_RISK` and `ISSUER` `TOTAL_RISK` on the weight proxy until benchmark-group and issuer-dimension evidence exist.
