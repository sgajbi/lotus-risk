...
7. repo-native RFC-0086 product and consumer declarations now live under
   `contracts/domain-data-products/`; `make domain-data-product-gate` and
   `make idea-opportunity-evidence-gate` are directly reachable from both blocking `check` and `ci`
   lanes,
...
16. `lotus-risk.idea-opportunity-runtime-evidence.v1` is a source-safe producer evidence pack for
    ...
    `BMK_PB_GLOBAL_BALANCED_60_40`, `2026-04-10`, and exact RFC-0076 canonical contract
    provenance for the platform demo-data contract and invariants. The artifact stores only digests
    and bounded supportability summaries, rejects non-ready/non-current receipts, validates
    canonical request digests, bounded summary values, and contract provenance, and clears only the
    corresponding Idea Risk source-proof blockers. It does not certify Idea persistence, data mesh,
...
6. `lotus-core` must be consumed as a governed source-data, analytics-input, snapshot/simulation, and support-metadata authority, while `lotus-performance` remains the authority for performance return and benchmark exposure context inputs — and, since the lotus-risk#291 consumer slice, for per-group return evidence consumed from its contribution surface (`/performance/contribution`, `ContributionRow.group_return`): only stateful `TOTAL_RISK` + `VOLATILITY` requests that evidence, per resolved period for `SECTOR` and `ASSET_CLASS`, using the Core-sourced expected group universe and the producer's source-owned reporting-currency valuation path (`BASE_ONLY`, not caller-invented FX). Core's missing dimension is canonical `UNKNOWN`; Performance's documented `emit.include_unclassified=true` spelling `Unclassified` maps only to that identity, while a real Core `Unclassified` category collision or a foreign producer group fails closed as `UPSTREAM_INVALID_RESPONSE`. It decomposes evidence only when every expected group covers every BUSINESS portfolio-return date (a missing date is unknown, never zero; no calculation date is zero-filled or dropped). Performance's extra in-range calendar-weekend contribution observations are validated but excluded from covariance; unlisted weekdays and out-of-window dates still fail closed, without inventing a bank-holiday calendar. It reports the per-set basis in `risk_basis`, refuses malformed nested payloads or unsupported economic bases as `UPSTREAM_INVALID_RESPONSE`, and keeps `POSITION`, `ISSUER`, unsupported `TOTAL_RISK` metrics, and `ACTIVE_RISK` on the weight proxy until compatible producer evidence exists. Performance's `BASE_ONLY` base/report currency label correction was delivered under lotus-performance#527 on main `f597e4d`; joint Risk consumer acceptance for that differing-currency path remains separate. Risk retains its mismatch refusal and does not relabel evidence or invent FX.
