...
6. `lotus-core` must be consumed as a governed source-data, analytics-input, snapshot/simulation, and support-metadata authority, while `lotus-performance` remains the authority for performance return and benchmark exposure context inputs — and, since the lotus-risk#291 consumer slice, for per-group return evidence consumed from its contribution surface (`/performance/contribution`, `ContributionRow.group_return`): stateful `TOTAL_RISK` requests that evidence per resolved period only for `SECTOR` and `ASSET_CLASS`, using the Core-sourced expected group universe and the producer's source-owned reporting-currency valuation path (`BASE_ONLY`, not caller-invented FX). It decomposes evidence only when every expected group covers every portfolio return date (a date absent from a group's evidence is unknown, never zero; nothing is zero-filled or dropped), reports the per-set basis in `risk_basis`, refuses malformed nested payloads or unsupported economic bases as `UPSTREAM_INVALID_RESPONSE`, and keeps `POSITION`, `ISSUER`, and `ACTIVE_RISK` on the weight proxy until compatible producer evidence exists.
