...
| `POST /analytics/risk/rolling-metrics` | Domain analytics | rolling historical risk diagnostics (volatility, Sharpe, beta, tracking error, information ratio, rolling max drawdown) | stateless + stateful | stateless + stateful + simulation | stateful sourcing via lotus-performance portfolio/benchmark/risk-free series contracts (as required by requested metrics) | lotus-report, lotus-gateway, PB/WM advisory channels | partial (simulation pending) |
| `POST /analytics/risk/historical-attribution` | Domain analytics | historical risk and active-risk attribution decomposition | stateless | stateless + stateful + simulation | stateless caller-supplied returns/exposures; planned stateful sourcing via lotus-performance portfolio/benchmark/exposure history contracts | lotus-report, lotus-gateway, PB/WM advisory workflows | partial (Slice A stateless implemented; stateful/simulation pending) |
