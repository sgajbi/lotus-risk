...
3. The latest quality baseline collects `451` tests across `91` Python test files.
...
1. Several service and contract modules remain large enough to deserve later cohesive extraction:
   `drawdown.py`, `attribution.py`, and targeted concentration/rolling/risk response fragments.
...
2. The latest generated baseline records `451` collected tests and `97%` total coverage.
