...
| 2 | Contract module size | Concentration, rolling, risk, drawdown, and attribution request/response contracts are split; concentration and rolling response models are further split into metric/detail and top-level response modules; drawdown response remains the largest response contract-size pressure | Split reusable metadata or nested contract fragments only where it improves reviewability and preserves OpenAPI output |
