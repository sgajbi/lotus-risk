...
| 2 | Contract module size | Concentration, rolling, risk, drawdown, and attribution request/response contracts are split; concentration, rolling, and drawdown response models are further split into metric/detail and top-level response modules; remaining contract-size pressure is concentrated in scenario, risk, and input modules | Split reusable metadata or nested contract fragments only where it improves reviewability and preserves OpenAPI output |
