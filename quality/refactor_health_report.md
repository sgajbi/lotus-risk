...
| 2 | Contract module size | Concentration, rolling, risk, drawdown, attribution, scenario, and mandate health request/response contracts are split; concentration, rolling, drawdown, and mandate health response models are further split into metric/detail and top-level response modules; remaining contract-size pressure is concentrated in risk and residual nested output modules | Split reusable metadata or nested contract fragments only where it improves reviewability and preserves OpenAPI output |
