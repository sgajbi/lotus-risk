...
| 2 | Contract module size | Concentration and rolling request/response contracts are split; `risk.py`, `drawdown.py`, and `attribution.py` remain prominent source files | Split reusable metadata or nested contract fragments only where it improves reviewability and preserves OpenAPI output |
