...
| 2 | Contract module size | Concentration, rolling, risk, drawdown, and attribution request/response contracts are split; rolling response models are further split into metric/window and period/response modules; concentration and drawdown response modules now carry the largest remaining contract-size pressure | Split reusable metadata or nested contract fragments only where it improves reviewability and preserves OpenAPI output |
