...
| 2 | Contract module size | Concentration, rolling, risk, drawdown, attribution, and scenario request/response contracts are split; risk input contracts now have shared, options, stateless, stateful, and request-envelope modules; risk output contracts now have metric, response-context, and response-envelope modules; rolling input contracts now have common, stateless, stateful, and request-envelope modules; rolling response contracts now have period, metadata, and response-envelope modules; drawdown response contracts now have period, metadata, and response-envelope modules; scenario output contracts now have common, governance, metadata, result, and response-envelope modules; concentration input contracts now have common, stateless, stateful, simulation, and request-envelope modules; attribution input contracts now have common, stateless, stateful, and request-envelope modules; attribution output contracts now have result, metadata, and response-envelope modules; concentration and drawdown response models are further split into metric/detail and top-level response modules; remaining contract-size pressure is concentrated in drawdown input/metric, risk-event-cohort, concentration metric/response, and rolling metric modules | Split reusable metadata or nested contract fragments only where it improves reviewability and preserves OpenAPI output |
