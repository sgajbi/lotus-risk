...
| 2 | Contract module size | Concentration, rolling, risk, and drawdown request/response contracts are split; `attribution.py` remains the largest prominent contract source file | Split reusable metadata or nested contract fragments only where it improves reviewability and preserves OpenAPI output |
