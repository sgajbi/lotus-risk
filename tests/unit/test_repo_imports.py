"""The repository's own src must win the import path over sibling apps.

The personal-path guard that grew here to a 1,051-line hand-written lexer
now lives at the right altitude as ``scripts/personal_path_guard.py`` (a
``tokenize`` pass; #254), gated from the blocking lanes and proven by
``tests/unit/test_personal_path_guard.py``.
"""
