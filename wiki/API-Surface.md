...
`GET /integration/capabilities` publishes what this deployment actually supports. The operating rule
for every downstream team is stated in [Home](./Home.md#current-posture) and repeated here because it
is the rule most often broken:
...
workflow behind it is supported for your portfolio, your metric, or your mode. The capability
...
A risk response carries not just a number but whether that number should be relied on. The
vocabulary is closed in three dimensions:
...
Because the sets are closed, a consumer can branch on them mechanically. The distinction that most
often matters is `insufficient_observations` versus `insufficient_aligned_observations`: the first
means there was not enough history, the second means portfolio and benchmark history did not overlap
enough — a different fix, and a different conversation with the data owner.

`empty` and `error` are not the same answer either. `empty` means the calculation ran and had
nothing to work on; `error` means it could not run.
