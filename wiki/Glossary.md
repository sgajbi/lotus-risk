...
| **`group_return_series_unavailable`** | historical attribution has no per-group return series, so the decomposition is a weight proxy rather than measured group risk. Returned on **every attribution response that produced a decomposition**, flagged or not; an `empty` response reports `no_return_observations` instead, having had nothing to decompose. Not a property of the request — a source limitation this service states rather than implies |
