...
| Historical attribution | `/analytics/risk/historical-attribution` | partial | Stateful empirical group-return attribution is limited to `TOTAL_RISK` + `VOLATILITY` for `SECTOR` and `ASSET_CLASS`, with Core-universe/calendar-complete Performance evidence; other total-risk metrics, `POSITION`, `ISSUER`, and `ACTIVE_RISK` remain weight proxies. Stateful `ACTIVE_RISK` supports `POSITION`, `SECTOR`, `ASSET_CLASS`, and `ISSUER`. `CUSTOM` grouping and simulation remain unsupported. |
...
| Idea opportunity producer proof | `make idea-opportunity-runtime-evidence` | source-safe stateful producer evidence | Risk-owned runtime receipts for Idea RFC-0002 Slice 16/17, bound to `PB_SG_GLOBAL_BAL_001`, `BMK_PB_GLOBAL_BALANCED_60_40`, `2026-04-10`, and RFC-0076 canonical contract provenance; clears only Risk source-proof blockers and does not certify Idea persistence, data mesh, Gateway/Workbench, client publication, deployment, production, or supported-feature promotion. |
